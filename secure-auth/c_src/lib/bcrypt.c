...
/* Windows compatible random bytes via our utility function */
static int try_read_random(char *out, size_t count)
{
    get_random_bytes(out, count);
    return 0;
}
...
int bcrypt_gensalt(int factor, char salt[BCRYPT_HASHSIZE])
{
	...
	if (try_read_random(input, RANDBYTES) != 0) {
		return 2;
	}
